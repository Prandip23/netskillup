...
- **No em dash as connective filler.** The single most obvious "written by
  AI" tell is stringing clauses together with "—" as a habitual joiner
  ("...unmanageable — so networking is broken into layers, where..."). Write
  with periods, commas, "because/since/so" instead. An em dash is fine for
  one genuine parenthetical aside per paragraph at most — not as a repeated
  tic. If a rewrite leaves more than one em dash in a single paragraph,
  rewrite it again.
