...
import {
  ...
  loadCatalog, publishedTopics, catalogErrors, relativeLink, applyReplacements,
} from "./site-lib.mjs";

export function renderCatalog(catalog) {
  return catalog.filter((module) => module.track === "networking").map((module) => {
...
<div class="module-head"><p class="module-label">MODULE ${module.number}</p><h2 id="${headingId}">${escapeHTML(module.title)}</h2><p>${escapeHTML(module.blurb)}</p><span class="module-count">${topics.length} topics</span></div>
...
<!-- site:metadata:end -->`;
  ...
  if (file === "index.html") {
    const modules = catalog.filter((module) => module.track === "networking" && module.topics.some((topic) => topic.status === "published"));
    const count = topics.filter((topic) => topic.module.track === "networking").length;
    ...
    replace(byId("curriculum-summary"), `${count} topics across ${modules.length} modules, from Ethernet frames to encrypted tunnels.`, true);
    replace(byId("search-results"), `${count} topics across ${modules.length} modules.`, true);
  } else {
    ...
    const home = topic.module.track === "networking" ? "index.html" : `${topic.module.track}/index.html`;
    const topicSearch = `<form class="search-box" id="reference-search" role="search" action="${relativeLink(file, home)}" method="get"><input type="search" name="q" placeholder="Search topics" aria-label="Search networking topics"><button class="search-submit" type="submit" aria-label="Search" title="Search">&#8594;</button></form>`;
...
export function syncSite(check = false) {
  ...
  const pages = ["index.html", ...publishedTopics(catalog).map((topic) => topic.file)];
  const outputs = new Map(pages.map((file) => [file, syncHTML(read(file), file, catalog, origin)]));
  outputs.set("sitemap.xml", `<?xml version="1.0" encoding="UTF-8"?>\n<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">\n${pages.map((file) => `  <url><loc>${escapeHTML(new URL(file === "index.html" ? "./" : file, `${origin}/`).href)}</loc></url>`).join("\n")}\n</urlset>\n`);
